...
public class MyHashMap<K, V> implements MyMap<K, V> {
    ...
    public V get(K key) {
        int bucketIndex = hash(key.hashCode(), 1);
    ...
    public V put(K key, V value) {
        ...
        if (get(key) != null) { // The key is already in the map
            while(table[hash(key.hashCode(), i)] != null) {
                i++;
            }
        }
        ...
        int bucketIndex = hash(key.hashCode(), i);
    ...
    private int hash(int hashCode, int i) {
        return (Math.abs(3 * hashCode + i) % 13) % 100;
    }

    @Override     /**Method to print information about hash map*/
    public void tableInfo() {
        System.out.println("Table Size: " + capacity +
                "\nNumber of Keys: " + size +
                "\nCurrent Load: " + (double)size/capacity +
                "\nLoad Threshold: " + loadFactorThreshold);
    }
